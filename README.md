...
- `Мягкий` — прежний admission-контроллер: один активный FakeTLS-handshake,
  после недавних успехов допускается два;
- `Тихий` — один активный handshake и более медленная последовательная выдача
  слотов;
- `Строгий` — один активный handshake, ограничение быстрых последовательных
  стартов `connect()`, паузы 3-6 секунд для очереди и cooldown после
  timeout/freeze.

Старый boolean `mtProxyHandshakeAdmission` мигрируется в `Мягкий` режим, если он
был включён. Новый режим хранится как `mtProxyConnectionPatternMode`,
передаётся в native через `setProxySettings` и перезапускает соединения при
изменении.

Когда режим не `Обычный`, задача планировщика:

- держать максимум одно активное FakeTLS-рукопожатие на адрес прокси, кроме
  мягкого режима после недавних успешных `server_hello_hmac_ok`;
...
логируются. В `Тихом` и `Строгом` режимах они дополнительно дают cooldown для
низкоприоритетных попыток, чтобы клиент не создавал быстрый повторяемый шум на
одном endpoint.
...
- `admission_grant`, `admission_queue`, `admission_release`,
  ...
  `admission_freeze_observed`, `admission_failure_cooldown`.
