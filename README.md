...
pre-ClientHello DNS/TCP-срывы включают короткую штрафную паузу уже для обычных
и медийных рукопожатий, а не только для скачивания/выгрузки. Это снижает шторм
повторных DNS/`connect()` по одному адресу прокси, но не смешивает сетевое
душение до ClientHello с JA4/ServerHello-ошибками.
